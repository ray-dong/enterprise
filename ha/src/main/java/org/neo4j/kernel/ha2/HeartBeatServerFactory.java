    ...
    implements ProtocolServerFactory
{
    ...
    public ProtocolServer newProtocolServer(Timeouts timeouts, MessageSource input, MessageProcessor output )
    {
        ...
        StateMachine<MultiPaxosContext, AcceptorMessage, AcceptorState> acceptor= new StateMachine<MultiPaxosContext, AcceptorMessage, AcceptorState>(context, AcceptorMessage.class, AcceptorState.start);
        StateMachine<MultiPaxosContext, ProposerMessage, ProposerState> coordinator= new StateMachine<MultiPaxosContext, ProposerMessage, ProposerState>(context, ProposerMessage.class, ProposerState.start);
        StateMachine<MultiPaxosContext, LearnerMessage, LearnerState> learner= new StateMachine<MultiPaxosContext, LearnerMessage, LearnerState>(context, LearnerMessage.class, LearnerState.start);
        StateMachine<MultiPaxosContext, AtomicBroadcastMessage, AtomicBroadcastState> paxosStateMachine= new StateMachine<MultiPaxosContext, AtomicBroadcastMessage, AtomicBroadcastState>(context, AtomicBroadcastMessage.class, AtomicBroadcastState.start);
